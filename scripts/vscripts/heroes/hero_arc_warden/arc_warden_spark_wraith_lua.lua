...
function OnProjectileHit( keys )
	...
	AddFOWViewer(caster:GetTeamNumber(), target:GetAbsOrigin(), ability:GetSpecialValueFor("vision_radius"), 3.34, true)
	modifier:Destroy()
end
...
function arc_warden_spark_wraith_thinker:OnIntervalThink()
	...
	elseif self.duration ~= nil then
		if GameRules:GetGameTime() > self.expire then
			...
			local enemies = FindUnitsInRadius(thinker:GetTeamNumber(), thinker_pos, nil, self.search_radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_CREEP + DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_NOT_MAGIC_IMMUNE_ALLIES, FIND_CLOSEST, false)
			if enemies[1] then
