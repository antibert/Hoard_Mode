...
function venomancer_plague_ward_datadriven_on_spell_start(keys)
	...
	if plague_ward_level >= 1 and plague_ward_level <= 4 then
		local plague_ward_unit = CreateUnitByName("npc_dota_venomancer_scourge_ward_" .. plague_ward_level, keys.target_points[1], false, keys.caster, keys.caster, keys.caster:GetTeam())
...
function modifier_plague_ward_datadriven_on_attack_landed(keys)
	if IsValidEntity(keys.attacker.venomancer_plague_ward_parent) then
		local poison_sting_ability = keys.attacker.venomancer_plague_ward_parent:FindAbilityByName("venomancer_poison_sting")
		if poison_sting_ability == nil then
