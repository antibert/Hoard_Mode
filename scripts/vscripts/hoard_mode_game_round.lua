--[[
	Choard_modeGameRound - A single round of hoard_mode
]]

if Choard_modeGameRound == nil then
	Choard_modeGameRound = class({})
end


function Choard_modeGameRound:ReadConfiguration( kv, gameMode, roundNumber )
	self._gameMode = gameMode
	self._nRoundNumber = roundNumber
	self._szRoundQuestTitle = kv.round_quest_title or "#DOTA_Quest_hoard_mode_Round"
	...
	for k, v in pairs( kv ) do
		if type( v ) == "table" and v.NPCName then
			local spawner = Choard_modeGameSpawner()
...
function Choard_modeGameRound:Precache()
	for _, spawner in pairs( self._vSpawners ) do
		spawner:Precache()
	end
end

function Choard_modeGameRound:Begin()
	self._vEnemiesRemaining = {}
	self._vEventHandles = {
		ListenToGameEvent( "npc_spawned", Dynamic_Wrap( Choard_modeGameRound, "OnNPCSpawned" ), self ),
		ListenToGameEvent( "entity_killed", Dynamic_Wrap( Choard_modeGameRound, "OnEntityKilled" ), self ),
		ListenToGameEvent( "dota_item_picked_up", Dynamic_Wrap( Choard_modeGameRound, 'OnItemPickedUp' ), self ),
		ListenToGameEvent( "dota_hoard_mode_revive_complete", Dynamic_Wrap( Choard_modeGameRound, 'Onhoard_modeReviveComplete' ), self )
	}
...
function Choard_modeGameRound:End()
	...
	FireGameEvent( "hoard_mode_show_round_end_summary", roundEndSummary )
end


function Choard_modeGameRound:Think()
	for _, spawner in pairs( self._vSpawners ) do
		spawner:Think()
	end
end
...
function Choard_modeGameRound:ChooseRandomSpawnInfo()
	return self._gameMode:ChooseRandomSpawnInfo()
end


function Choard_modeGameRound:IsFinished()
	for _, spawner in pairs( self._vSpawners ) do
...
function Choard_modeGameRound:GetXPPerCoreUnit()
...
function Choard_modeGameRound:OnNPCSpawned( event )
...
function Choard_modeGameRound:OnEntityKilled( event )
	...
	if killedUnit.hoard_mode_IsCore then
...
function Choard_modeGameRound:Onhoard_modeReviveComplete( event )
...
function Choard_modeGameRound:OnItemPickedUp( event )
...
function Choard_modeGameRound:_CheckForGoldBagDrop( killedUnit )
...
function Choard_modeGameRound:StatusReport( )
