...
function modifier_duel_datadriven_on_death(keys)
	...
	local reward_damage = keys.RewardDamage

	local hTalent = caster:FindAbilityByName( "special_bonus_unique_legion_commander" )
	if hTalent and hTalent:GetLevel() > 0 then
		reward_damage = reward_damage + hTalent:GetSpecialValueFor("value")
	end
	...
	if unit == caster then  --If Legion Commander was killed.
		...
		for i, individual_hero in ipairs(herolist) do  --Iterate through the enemy heroes, award any with a Duel modifier the reward damage, and then remove that modifier.
			if individual_hero:GetTeam() ~= caster_team and individual_hero:HasModifier(modifier_duel) then
				if individual_hero:HasModifier(modifier_duel) then
					...
					local duel_stacks = individual_hero:GetModifierStackCount(modifier_duel_damage, ability) + reward_damage
		...
		local duel_stacks = caster:GetModifierStackCount(modifier_duel_damage, ability) + reward_damage
