  ...
  for _,hero in pairs(heroes) do
    if hero:HasModifier("modifier_meepo_divided_we_stand") then
      ...
      if hero.firstMeepo then
        ...
        -- give the clone prime's boot if they dont already have it
        if currentBoot then 
          local itemFound = nil
          if TableCount(primeItems) > 0 then
            ...
            if itemFound then
              -- remove clone's current boot
              itemFound:Destroy()
              -- grab the slot of boot in meepo prime's inventory
              ...
              hero:SwapItems(slot, hero:GetSlot(newboot))
            end
          end
        end
      end
